...
var PointsTable = function (_a) {
    var rows = _a.rows, onViewTeam = _a.onViewTeam;
    return (<div className="bg-slate-900 border border-slate-800 rounded-[2rem] overflow-hidden shadow-xl">
      <div className="w-full">
        <table className="w-full text-left border-collapse table-fixed md:table-auto">
          ...
          <tbody className="divide-y divide-slate-800/50">
            {rows.map(function (r, idx) { return (<tr key={r.teamId} className="group hover:bg-white/5 transition-colors">
                <td className="pl-4 pr-2 py-2 md:px-6 md:py-3 overflow-hidden">
                  <div className="flex items-center gap-2 md:gap-3">
                    ...
                    {onViewTeam ? (<button onClick={function () { return onViewTeam(r.teamId); }} className="text-[11px] md:text-sm font-black text-slate-200 group-hover:text-white transition-colors hover:underline truncate block w-full text-left leading-tight">
                        {r.teamName}
                      </button>) : (<span className="text-[11px] md:text-sm font-black text-slate-200 group-hover:text-white transition-colors truncate block leading-tight">
                        {r.teamName}
                      </span>)}
                  </div>
                </td>
                ...
                <td className={"px-1 py-2 text-center font-bold text-[9px] md:text-xs md:px-4 md:py-3 ".concat(r.nrr >= 0 ? 'text-emerald-400' : 'text-red-400')}>
                  {r.nrr > 0 ? '+' : ''}{r.nrr}
                </td>
      ...
      {rows.length === 0 && (<div className="p-10 text-center flex flex-col items-center gap-2 opacity-20">
