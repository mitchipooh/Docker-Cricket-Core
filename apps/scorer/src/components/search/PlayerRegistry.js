...
var PlayerRegistry = function (_a) {
    var allPlayers = _a.allPlayers, allTeams = _a.allTeams, onViewPlayer = _a.onViewPlayer, onBack = _a.onBack;
    var _b = (0, react_1.useState)(''), searchTerm = _b[0], setSearchTerm = _b[1];
    var _c = (0, react_1.useState)('ALL'), selectedTeamId = _c[0], setSelectedTeamId = _c[1];
    var filteredPlayers = (0, react_1.useMemo)(function () {
        return allPlayers.filter(function (player) {
            var matchesSearch = player.name.toLowerCase().includes(searchTerm.toLowerCase());
            // Find player's team
            var playerTeam = allTeams.find(function (t) { return t.players.some(function (p) { return p.id === player.id; }); });
            var matchesTeamName = playerTeam && playerTeam.name.toLowerCase().includes(searchTerm.toLowerCase());
            var matchesTeam = selectedTeamId === 'ALL' || (playerTeam && playerTeam.id === selectedTeamId);
            return (matchesSearch || matchesTeamName) && matchesTeam;
        });
    }, [allPlayers, allTeams, searchTerm, selectedTeamId]);
    // Group players by role for stats
    var stats = {
        total: filteredPlayers.length,
        batsmen: filteredPlayers.filter(function (p) { return p.role === 'Batsman'; }).length,
        bowlers: filteredPlayers.filter(function (p) { return p.role === 'Bowler'; }).length,
        allRounders: filteredPlayers.filter(function (p) { return p.role === 'All-rounder'; }).length,
        keepers: filteredPlayers.filter(function (p) { return p.role === 'Wicket-keeper'; }).length,
        runs: filteredPlayers.reduce(function (acc, p) { return acc + p.stats.runs; }, 0),
        wickets: filteredPlayers.reduce(function (acc, p) { return acc + p.stats.wickets; }, 0),
    };
    return (<div className="max-w-[100vw] overflow-x-hidden mx-auto p-4 md:p-12 min-h-screen animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 md:mb-12 gap-6">
                ...
                <div className="flex items-center gap-4 bg-white p-2 rounded-2xl shadow-lg border border-slate-100 w-full md:w-auto">
                    ...
                    <select value={selectedTeamId} onChange={function (e) { return setSelectedTeamId(e.target.value); }} className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 focus:outline-none focus:border-indigo-500 transition-all cursor-pointer hover:bg-slate-100 flex-1 md:flex-none">
                        <option value="ALL">All Teams</option>
                        {allTeams.map(function (team) { return (<option key={team.id} value={team.id}>{team.name}</option>); })}
                    </select>
                </div>
            </div>
            ...
            var team = allTeams.find(function (t) { return t.players.some(function (p) { return p.id === player.id; }); });
            return (<div key={player.id} onClick={function () { return onViewPlayer(player.id); }} className="bg-white rounded-[2rem] p-6 border border-slate-100 hover:border-indigo-300 hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer group relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-4 mb-6 relative">
                                ...
                                <div>
                                    ...
                                    {team && (<span className="inline-block mt-1 px-2 py-0.5 bg-slate-100 rounded-md text-[9px] font-bold text-slate-500 uppercase tracking-wide">
                                            {team.name}
                                        </span>)}
                                </div>
                            </div>
            ...
            {filteredPlayers.length === 0 && (<div className="text-center py-20">
