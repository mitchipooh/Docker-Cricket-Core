...
var TeamRegistry = function (_a) {
    var allTeams = _a.allTeams, allOrganizations = _a.allOrganizations, onViewTeam = _a.onViewTeam, onBack = _a.onBack;
    var _b = (0, react_1.useState)(''), searchTerm = _b[0], setSearchTerm = _b[1];
    var _c = (0, react_1.useState)('ALL'), selectedOrgId = _c[0], setSelectedOrgId = _c[1];
    var filteredTeams = (0, react_1.useMemo)(function () {
        return allTeams.filter(function (team) {
            ...
            var teamOrg = allOrganizations.find(function (org) { var _a; return (_a = org.memberTeams) === null || _a === void 0 ? void 0 : _a.some(function (mt) { return mt.id === team.id; }); });
            var matchesOrg = selectedOrgId === 'ALL' || (teamOrg && teamOrg.id === selectedOrgId);
    ...
    var stats = {
        ...
        totalPlayers: filteredTeams.reduce(function (acc, t) { return acc + t.players.length; }, 0),
        totalRuns: filteredTeams.reduce(function (acc, t) { return acc + t.players.reduce(function (sum, p) { return sum + p.stats.runs; }, 0); }, 0),
        totalWickets: filteredTeams.reduce(function (acc, t) { return acc + t.players.reduce(function (sum, p) { return sum + p.stats.wickets; }, 0); }, 0),
        totalMatches: filteredTeams.reduce(function (acc, t) { return acc + t.players.reduce(function (sum, p) { return sum + p.stats.matches; }, 0); }, 0),
    };
    return (<div className="max-w-[100vw] overflow-x-hidden mx-auto p-4 md:p-12 min-h-screen animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 md:mb-12 gap-6">
                ...
                <div className="flex items-center gap-4 bg-white p-2 rounded-2xl shadow-lg border border-slate-100 w-full md:w-auto">
                    <div className="relative flex-1 md:flex-none">
                        ...
                        <input type="text" placeholder="Search by team name..." value={searchTerm} onChange={function (e) { return setSearchTerm(e.target.value); }} className="bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-sm font-bold text-slate-700 w-full md:w-64 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all"/>
                    </div>
                    <select value={selectedOrgId} onChange={function (e) { return setSelectedOrgId(e.target.value); }} className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-700 focus:outline-none focus:border-indigo-500 transition-all cursor-pointer hover:bg-slate-100 flex-1 md:flex-none">
                        <option value="ALL">All Organizations</option>
                        {allOrganizations.map(function (org) { return (<option key={org.id} value={org.id}>{org.name}</option>); })}
                    </select>
                </div>
            </div>
            ...
            var org = allOrganizations.find(function (o) { var _a; return (_a = o.memberTeams) === null || _a === void 0 ? void 0 : _a.some(function (mt) { return mt.id === team.id; }); });
            var teamRuns = team.players.reduce(function (sum, p) { return sum + p.stats.runs; }, 0);
            var teamWickets = team.players.reduce(function (sum, p) { return sum + p.stats.wickets; }, 0);
            var teamMatches = team.players.reduce(function (sum, p) { return sum + p.stats.matches; }, 0);
            var avgRuns = teamMatches > 0 ? (teamRuns / teamMatches).toFixed(0) : 0;
            return (<div key={team.id} onClick={function () { return onViewTeam(team.id); }} className="bg-white rounded-[2rem] p-6 border border-slate-100 hover:border-indigo-300 hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer group relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-4 mb-6 relative">
                                ...
                                <div className="flex-1">
                                    ...
                                    {org && (<span className="inline-block mt-1 px-2 py-0.5 bg-slate-100 rounded-md text-[9px] font-bold text-slate-500 uppercase tracking-wide">
                                            {org.name}
                                        </span>)}
                                </div>
                            </div>
            ...
            {filteredTeams.length === 0 && (<div className="text-center py-20">
