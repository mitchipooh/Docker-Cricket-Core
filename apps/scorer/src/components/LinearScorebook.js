...
exports.LinearScorebook = void 0;
var react_1 = require("react");
var LinearScorebook = function (_a) {
    var data = _a.data;
    var rows = data.rows, extras = data.extras, totalScore = data.totalScore, overs = data.overs, runRate = data.runRate, fow = data.fow, didNotBat = data.didNotBat, _b = data.bowlingRows, bowlingRows = _b === void 0 ? [] : _b;
    var getBowlerName = function (row) {
        return row.bowlerName || '';
    };
    // --- Symbol Generators ---
    var renderBallSymbol = function (b) {
        var symbol = (<span className="font-bold text-slate-400">.</span>);
        if (b.isWicket) {
            symbol = <span className="font-black text-red-600">W</span>;
        }
        else if (b.extraType === 'Wide') {
            symbol = (<div className="relative w-full h-full flex items-center justify-center">
    ...
    return (<div className="bg-white rounded-none md:rounded-[4px] border border-slate-300 shadow-xl overflow-hidden font-mono text-xs text-slate-800 max-w-6xl mx-auto">
         ...
         <div className="w-full overflow-x-auto">
            <table className="w-full text-left border-collapse">
               ...
               <tbody>
                  {/* 1. Batted Players */}
                  {rows.map(function (row, idx) { return (<tr key={row.playerId} className="border-b border-slate-200 hover:bg-yellow-50/30 transition-colors h-12">
                        ...
                        <td className="px-2 border-r border-slate-300 bg-white align-middle">
                           <div className="flex flex-wrap items-center gap-1 leading-none py-1">
                              ...
                              {row.scoringSequence.map(function (score, sIdx) { return (<div key={sIdx} className={"\n                               flex items-center justify-center w-5 h-5 text-[10px] font-bold rounded-sm border select-none\n                               ".concat(score === '•' ? 'border-slate-100 text-slate-400' :
                  ...
                  {/* 3. Did Not Bat */}
                  {didNotBat.map(function (p, idx) { return (<tr key={p.id} className="border-b border-slate-200 h-10 opacity-60">
         ...
         {/* --- FALL OF WICKETS --- */}
         {fow.length > 0 && (<div className="border-b-2 border-slate-800 p-4 bg-[#fdfbf7]">
               <h4 className="text-[10px] font-black uppercase tracking-widest mb-2 text-slate-500">Fall of Wickets</h4>
               <div className="flex flex-wrap gap-4">
                  {fow.map(function (f, i) { return (<div key={i} className="flex flex-col items-center border border-slate-300 bg-white px-3 py-1 rounded-sm shadow-sm">
               ...
               <div className="w-full overflow-x-auto">
                  ...
                  <div className="flex border-b border-slate-300 bg-slate-100">
                     ...
                     <div className="flex-1 flex">
                        {Array.from({ length: 12 }).map(function (_, i) { return (<div key={i} className="w-16 shrink-0 border-r border-slate-200 text-center p-1 text-[9px] text-slate-400">Ov {i + 1}</div>); })}
                  ...
                  {/* Grid Rows */}
                  {bowlingRows.map(function (bowler) { return (<div key={bowler.playerId} className="flex border-b border-slate-300 h-14">
                        {/* Name */}
                        ...
                        <div className="flex-1 flex bg-white overflow-hidden">
                           {bowler.oversHistory.map(function (overBalls, idx) { return (<div key={idx} className="w-16 shrink-0 border-r border-slate-200 border-dashed flex flex-wrap content-start p-0.5 gap-px relative">
                                 {/* Grid inside the box for 6 balls */}
                                 {overBalls.map(function (ball, bIdx) { return (<div key={bIdx} className="w-4 h-4 flex items-center justify-center text-[9px] bg-slate-50 border border-slate-100 rounded-[2px]" title={ball.commentary}>
                           ...
                           {Array.from({ length: Math.max(0, 12 - bowler.oversHistory.length) }).map(function (_, i) { return (<div key={"empty-".concat(i)} className="w-16 shrink-0 border-r border-slate-100 border-dashed bg-slate-50/30"></div>); })}
                        </div>
                  ...
                  {bowlingRows.length === 0 && (<div className="p-8 text-center text-slate-400 text-xs uppercase italic">
...
exports.LinearScorebook = LinearScorebook;
