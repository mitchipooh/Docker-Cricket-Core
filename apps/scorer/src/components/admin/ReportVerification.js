...
var ReportVerification = function (_a) {
    var submission = _a.submission, fixture = _a.fixture, onApprove = _a.onApprove, onReject = _a.onReject, onBack = _a.onBack;
    var _b = (0, react_1.useState)(''), feedback = _b[0], setFeedback = _b[1];
    var _c = (0, react_1.useState)(false), isRejecting = _c[0], setIsRejecting = _c[1];
    return (<div className="max-w-7xl mx-auto p-6 md:p-12 h-full overflow-y-auto custom-scrollbar animate-in fade-in duration-700">
            <div className="flex items-center justify-between mb-12">
                ...
                <div className="flex gap-4">
                    <button onClick={function () { return setIsRejecting(true); }} className="px-8 py-4 bg-white text-red-500 border border-red-100 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg hover:bg-red-50 transition-all">
                        Reject Report
                    </button>
                    <button onClick={function () { return onApprove(submission.id); }} className="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-2xl hover:bg-indigo-500 transition-all">
                        Verify & Update Stats
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 pb-20">
                ...
                <div className="space-y-6">
                    <div className="bg-slate-900 rounded-[3rem] p-4 shadow-2xl overflow-hidden aspect-[3/4] group relative">
                        {submission.scorecardPhotoUrl ? (submission.scorecardPhotoUrl.includes('application/pdf') || submission.scorecardPhotoUrl.toLowerCase().endsWith('.pdf') ? (<iframe src={submission.scorecardPhotoUrl} className="w-full h-full rounded-[2rem] bg-white" title="Scorecard PDF"/>) : (<img src={submission.scorecardPhotoUrl} className="w-full h-full object-contain rounded-[2rem] group-hover:scale-105 transition-transform duration-700 cursor-zoom-in" alt="Scorecard"/>)) : (<div className="w-full h-full flex flex-col items-center justify-center text-white/20">
                                <span className="text-6xl mb-4">📷</span>
                                <p className="font-black uppercase text-xs tracking-widest">No scorecard photo attached</p>
                            </div>)}
                        <a href={submission.scorecardPhotoUrl} target="_blank" rel="noopener noreferrer" className="absolute bottom-6 right-6 bg-white/10 backdrop-blur-md text-white px-4 py-2 rounded-lg font-bold text-xs uppercase hover:bg-white/20 transition-colors">
                            Open Original ↗
                        </a>
                    </div>
                        ...
                        <div className="max-h-[40vh] overflow-y-auto pr-4 custom-scrollbar">
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {submission.playerPerformances.map(function (p) { return (<tr key={p.playerId} className="group">
            ...
            {isRejecting && (<div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[100] flex items-center justify-center p-6">
                    <div className="bg-white w-full max-w-lg rounded-[3rem] p-12 shadow-2xl animate-in zoom-in duration-300">
                        ...
                        <textarea value={feedback} onChange={function (e) { return setFeedback(e.target.value); }} placeholder="e.g. Scanned photo is blurry, or runs total does not match..." className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-6 font-bold text-slate-700 outline-none focus:ring-2 focus:ring-red-500 h-40 resize-none mb-8"/>
                        <div className="flex gap-4">
                            <button onClick={function () { return setIsRejecting(false); }} className="flex-1 py-4 text-slate-400 font-black uppercase text-xs tracking-widest">Nevermind</button>
                            <button onClick={function () { return onReject(submission.id, feedback); }} disabled={!feedback} className="flex-[2] py-4 bg-red-500 text-white rounded-xl font-black uppercase text-xs tracking-widest shadow-xl disabled:opacity-50">
                                Send Rejection
                            </button>
                        </div>
                    </div>
...
exports.ReportVerification = ReportVerification;
