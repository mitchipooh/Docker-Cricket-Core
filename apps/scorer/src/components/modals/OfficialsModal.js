...
exports.OfficialsModal = void 0;
var react_1 = require("react");
var OfficialsModal = function (_a) {
    var isOpen = _a.isOpen, onClose = _a.onClose, currentUmpires = _a.currentUmpires, availableOfficials = _a.availableOfficials, onSave = _a.onSave;
    var _b = (0, react_1.useState)([]), selectedIds = _b[0], setSelectedIds = _b[1];
    var _c = (0, react_1.useState)(''), guestName = _c[0], setGuestName = _c[1];
    ...
    var handleToggle = function (id) {
        if (selectedIds.includes(id)) {
            setSelectedIds(selectedIds.filter(function (uid) { return uid !== id; }));
        }
        else {
            ...
            setSelectedIds(__spreadArray(__spreadArray([], selectedIds, true), [id], false));
        }
    };
    var handleAddGuest = function () {
        if (guestName.trim()) {
    ...
    return (<div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[300] flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl animate-in zoom-in-95 overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="p-6 flex-1 overflow-y-auto custom-scrollbar space-y-4">
           {availableOfficials.length === 0 && (<div className="text-center py-8 text-slate-400 text-xs font-bold uppercase tracking-widest">
                   No officials found in this organization.
               </div>)}
           
           <div className="grid grid-cols-1 gap-2">
               {availableOfficials.map(function (official) {
            var isSelected = selectedIds.includes(official.id);
            return (<button key={official.id} onClick={function () { return handleToggle(official.id); }} className={"p-4 rounded-xl border flex items-center justify-between transition-all ".concat(isSelected ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'bg-slate-50 border-slate-100 text-slate-600 hover:bg-slate-100')}>
                           <div className="flex items-center gap-3">
                               <div className={"w-10 h-10 rounded-full flex items-center justify-center font-black ".concat(isSelected ? 'bg-white text-indigo-600' : 'bg-white border border-slate-200')}>
                                   {official.name.charAt(0)}
                               </div>
                               <div className="text-left">
                                   <div className="font-bold text-sm">{official.name}</div>
                                   <div className={"text-[10px] uppercase font-bold ".concat(isSelected ? 'text-indigo-200' : 'text-slate-400')}>ID: {official.handle}</div>
                               </div>
                           </div>
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50 flex gap-4">
           ...
           <button onClick={function () { onSave(selectedIds); onClose(); }} className="flex-1 py-4 bg-emerald-500 text-white rounded-xl font-black uppercase text-xs tracking-widest shadow-xl hover:bg-emerald-400 transition-all">
...
exports.OfficialsModal = OfficialsModal;
