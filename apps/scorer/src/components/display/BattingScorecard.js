...
exports.BattingScorecard = void 0;
var react_1 = require("react");
var BattingScorecard = function (_a) {
    var data = _a.data;
    var rows = data.rows, extras = data.extras, totalScore = data.totalScore, overs = data.overs, runRate = data.runRate, fow = data.fow, didNotBat = data.didNotBat;
    return (<div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-xl flex flex-col w-full">
      <div className="overflow-x-auto no-scrollbar">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-800/50">
            {rows.map(function (r) { return (<tr key={r.playerId} className={"group hover:bg-white/5 transition-colors ".concat(r.atCrease ? 'bg-indigo-500/5' : '')}>
                <td className="px-3 py-1.5">
                  <div className="flex flex-col">
                    <span className={"text-[11px] font-black ".concat(r.atCrease ? 'text-indigo-400' : 'text-slate-200')}>
                      {r.name}{r.atCrease ? '*' : ''}
                    </span>
      ...
      {/* Did Not Bat */}
      {didNotBat.length > 0 && (<div className="px-3 py-2 border-t border-slate-800 bg-slate-900">
              <div className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Did Not Bat</div>
              <div className="text-[10px] text-slate-400 font-medium leading-relaxed">
                  {didNotBat.map(function (p) { return p.name; }).join(', ')}
              </div>
          </div>)}

      {/* Fall of Wickets */}
      {fow.length > 0 && (<div className="px-3 py-2 border-t border-slate-800 bg-slate-900/50">
              <div className="text-[8px] font-black text-slate-500 uppercase tracking-widest mb-1">Fall of Wickets</div>
              <div className="flex flex-wrap gap-1.5">
                  {fow.map(function (f, i) { return (<span key={i} className="text-[9px] text-slate-300 bg-slate-800 px-1.5 py-0.5 rounded border border-slate-700">
...
exports.BattingScorecard = BattingScorecard;
