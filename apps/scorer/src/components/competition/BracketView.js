...
var BracketView = function (_a) {
    var fixtures = _a.fixtures, onViewMatch = _a.onViewMatch;
    var semiFinals = fixtures.filter(function (f) { return f.stage === 'Semi-Final'; });
    var final = fixtures.find(function (f) { return f.stage === 'Final'; });
    var MatchCard = function (_a) {
        var match = _a.match, title = _a.title;
        if (!match)
            return (<div className="w-64 h-32 bg-slate-800/50 rounded-2xl border-2 border-dashed border-slate-700 flex items-center justify-center">
            <span className="text-slate-500 font-bold text-xs uppercase tracking-widest">{title} TBD</span>
        </div>);
        var isWinnerA = match.winnerId && match.winnerId === match.teamAId;
        var isWinnerB = match.winnerId && match.winnerId === match.teamBId;
        return (<div onClick={function () { return onViewMatch(match); }} className={"w-64 bg-slate-800 rounded-2xl border transition-all cursor-pointer relative overflow-hidden group ".concat(match.status === 'Live' ? 'border-red-500 shadow-[0_0_15px_rgba(220,38,38,0.3)]' : 'border-slate-700 hover:border-indigo-500')}>
        ...
        <div className="p-4 space-y-2">
            <div className={"flex justify-between items-center ".concat(isWinnerA ? 'text-emerald-400' : isWinnerB ? 'text-slate-500' : 'text-white')}>
            ...
            <div className={"flex justify-between items-center ".concat(isWinnerB ? 'text-emerald-400' : isWinnerA ? 'text-slate-500' : 'text-white')}>
    ...
    return (<div className="flex flex-col md:flex-row items-center justify-center gap-12 p-8 overflow-x-auto min-h-[400px]">
       ...
       <div className="flex flex-col items-center gap-6">
          ...
          {(final === null || final === void 0 ? void 0 : final.winnerId) && (<div className="bg-gradient-to-r from-amber-400 to-yellow-600 text-white px-6 py-2 rounded-full font-black text-xs uppercase tracking-widest shadow-xl animate-in zoom-in">
